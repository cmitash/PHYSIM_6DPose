...
class MatchSuper4PCSImpl {
 ...
 private:
  ...
  // PQP 
  PQP_Model *ObjectModelPQP;
  PQP_Model *ShelfModelPQP;
  std::string objname;
...
MatchSuper4PCSImpl::Verify(const Eigen::Matrix<Scalar, 4, 4>& mat) {
  //Chaitanya Debug
  //std::cout<<"Verifying Transform "<<std::endl;
#ifdef TEST_GLOBAL_TIMINGS
    Timer t_verify (true);
#endif
...
//#define STATIC_BASE
  ...
  // Centroid of the basis, computed once and using only the three first      
  ...
  for (int i = 0; i < congruent_quads.size(); ++i) {
    ...
    if (ok && rms >= Scalar(0.)) {
      ...
      if (rms < distance_factor * options_.delta) {
        ...
        // Chaitanya
        // Terminate if time expires.
        float fraction_time = static_cast<float>(clock() - t0) / CLOCKS_PER_SEC /
                                  options_.max_time_seconds;
        // std::cout<<"fraction time : "<<fraction_time<<std::endl;
        if(fraction_time > 0.99){
  ...
  //Chaitanya Debug
  // std::cout<<"TryOneBase Iteration : "<<std::endl;
...
void MatchSuper4PCSImpl::Initialize(const std::vector<Point3D>& P,
  ...
  if (Q.size() > options_.sample_size){
      ...
      //Chaitanya Debug
      std::cout<<"Q sample fraction is : "<< sample_fraction_Q<<std::endl;
  ...
  //Collision checking
  // std::string ObjModel = "/home/pracsys/github/PHYSIM_6DPose/ros-packages/src/physics-scene-rendering-vision/obj_models_physics/" + objname;
  // ObjectModelPQP = iomananger1.ReadPQPModel((char *)ObjModel.c_str());
}
...
void MatchSuper4PCSImpl::setInputName(std::string inputname){
  objname = inputname.substr(87);
  objname.replace(objname.size()-3,3,"obj");
...
void MatchSuper4PCS::setInputName(std::string inputname){
      pimpl_->setInputName(inputname);
}
}
