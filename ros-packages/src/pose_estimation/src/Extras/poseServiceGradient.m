...
for obIdx = 1:size(sceneData.objects,2)
    ...
    pclname = sprintf('rcnn-clean-%d',obIdx);
    pclname = fullfile(scenePath, pclname);
    pcwrite(objSegCloud,pclname,'PLYFormat','binary');
...
% Gradient Optimization : Vision and Physics
numItr = 1;
for iter = 1:numItr
    fprintf('Iteration : %d\n', iter);
    ...
    [objName, val1, val2, val3, val4, ...
     val5, val6, val7, val8, ...
     val9, val10, val11, val12, val13, val14, val15] = textread(init_pose, '%s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n');
    ...
    fprintf('[Processing] compute vision cost\n');
    ...
    for obIdx = 1:size(sceneData.objects,2)
        ...
        vis_score = computegradient(predObjPoseWorld, sceneData.extBin2World, objModelPts, scenePath, obIdx);
end
